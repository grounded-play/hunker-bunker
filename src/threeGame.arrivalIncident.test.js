...
const METHODS = ['setActiveExpedition', 'syncExpeditionBountyTracker', 'armArrivalIncident', 'updateArrivalIncident',
    'spawnArrivalIncident', 'spawnArrivalMember', 'dropArrivalCache', 'clearCrashSiteDebris', 'placeCrashSiteDebris'];
...
function game({ multiplayer = false } = {}) {
    ...
    const g = {
        ...
        scatterMaterials: { cryosnail: {}, cybersnail: {}, bunker_junk: {}, bunker_junk_uncommon: {}, prop_bunker_supplies: {} },
        ...
        createScatterInstance: (placement) => ({
            ...
            userData: { type: placement.type, isElite: placement.spawnedElite },
...
describe('the arrival incident in the runtime', () => {
    it('waits, announces the pack, tracks it, and leaves a salvage cache when it is cleared', () => {
        const g = game();
        const pack = (x) => x.scatterSprites.filter((s) => s.userData.arrivalIncident);
        g.setActiveExpedition(gale(2));
        g.updateArrivalIncident(10);
        expect(pack(g)).toHaveLength(0);
        g.updateArrivalIncident(11);
        expect(pack(g).map((s) => s.userData.type)).toEqual(['cryosnail', 'cryosnail', 'cryosnail']);
        expect(pack(g).every((s) => s.userData.aiMode === 'hunt' && s.userData.arrivalIncident)).toBe(true);
        for (const sprite of pack(g)) {
        ...
        pack(g)[0].userData.burstTriggered = true;
        g.updateArrivalIncident(0.1);
        expect(g.pickupMeshes).toHaveLength(0);
        for (const sprite of pack(g)) sprite.userData.burstTriggered = true;
        g.updateArrivalIncident(0.1);
    ...
    it('pays nothing when the pack is unloaded rather than beaten', () => {
        ...
        for (const sprite of g.scatterSprites.filter((s) => s.userData.arrivalIncident)) sprite.parent = null;
        g.updateArrivalIncident(0.1);
        expect(g.pickupMeshes).toHaveLength(0);
    ...
    it('holds off outside live gameplay and stays out of multiplayer', () => {
        ...
        expect(g.scatterSprites.filter((s) => s.userData.arrivalIncident)).toHaveLength(0);
...
describe('crash-site wreckage varies each landing', () => {
    const debris = (g) => g.scatterSprites.filter((s) => s.userData.crashDebris);

    it('scatters three pieces clear of the wreck, the operator and the door lane', () => {
        const g = game();
        g.setActiveExpedition(gale(2));
        g.updateArrivalIncident(0.01);
        const placed = debris(g);
        expect(placed).toHaveLength(3);
        for (const piece of placed) {
            expect(Math.hypot(piece.position.x - 9, piece.position.z - 11)).toBeGreaterThan(3.5);
            expect(Math.hypot(piece.position.x - 9, piece.position.z - 12)).toBeGreaterThan(3);
            expect(piece.position.x > 6.5 && piece.position.x < 11.5 && piece.position.z < 6).toBe(false);
        }
    });

    it('changes with the deployment and never piles up across deployments', () => {
        const g = game();
        g.setActiveExpedition(gale(2));
        g.updateArrivalIncident(0.01);
        const first = debris(g).map((s) => `${s.userData.type}@${s.position.x},${s.position.z}`).sort();
        g.setActiveExpedition({ ...gale(3), expeditionSeed: 99 });
        g.updateArrivalIncident(0.01);
        const second = debris(g).map((s) => `${s.userData.type}@${s.position.x},${s.position.z}`).sort();
        expect(second).toHaveLength(3);
        expect(second).not.toEqual(first);
        expect(g.scatterSprites.filter((s) => s.userData.crashDebris && s.parent)).toHaveLength(3);
    });
});
