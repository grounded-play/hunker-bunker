...
Last updated: 2026-08-21 (Sprint 28 log13 gameplay-FPS mitigation).
...
| Performance | Prior chunk-mount batching, stale attribution, and collapsed-container render waste are fixed. The log13 worktree now holds shader programs constant during movement, prewarms normal/adaptive render variants, and removes the measured update-loop bottlenecks by sharing one containment snapshot per scatter frame and replacing a per-frame whole-document overlay selector. The controlled headless movement profile improved from 51.40ms/19.5 FPS to 29.80ms/33.6 FPS while retaining 488 scatter objects/34 enemies; shader programs stayed 100 -> 100. Steam Deck/adaptive mode still bypasses two tilt-shift passes, disables shadows, and caps pixel ratio at 0.85 without hiding chunks or actors. Full suite/build are green (239 files, 1,941 tests); a new packaged two-player Deck capture remains the acceptance gate. See `docs/log13-gameplay-fps-plan-2026-08-20.md`, `docs/log13-gameplay-fps-findings-and-fix-plan-2026-08-20.md`, and Sprint 28 Lane F. |
