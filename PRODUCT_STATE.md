...
| PvP | One physical Deck capture shows Steam lobby/ready/deploy, remote-avatar readiness, inbound `pvp-rival` damage, one death, and respawn. Current source fixes PvP at 4 hearts independent of fatigue/loadout, adds 3.0s spawn protection, blocks Black Box death exploitation, adds client skip/server validation rejection for PvP leaderboards, establishes debounced monotonic door sequencing, adds outgoing hit telemetry, and bypasses PvE missions/cards; focused unit tests pass. | Not launch-ready: the capture predates these source fixes. Require paired bidirectional hit/result/reconnect and deployed-backend evidence; paired physical Deck verification remains open. |
