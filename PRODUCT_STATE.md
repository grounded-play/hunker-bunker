...
| PvP | One physical Deck capture shows Steam lobby/ready/deploy, remote-avatar readiness, inbound `pvp-rival` damage, one death, and respawn. Current source adds local four-heart/fatigue exclusion, spawn protection, Black Box suppression, leaderboard guards, door sequencing, hit telemetry, and PvE mission/card bypass; the full automated suite passes. | **P0 authority mismatch:** local PvP vitals use four hearts, while the relay and newly created remote replicas still initialize at three. Align one shared HP contract and prove it with an integration test before a package run. The capture predates all source changes; paired bidirectional hit/result/reconnect, deployed-backend, and Deck evidence remain open. |
