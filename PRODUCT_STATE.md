...
| Steam Lobby (Friends/Invite/Join Game) | **Code-complete** 2026-08-20, all 4 implementation steps of `docs/steam-lobby-integration-plan-2026-08-20.md` shipped (single-instance lock + `+connect_lobby` parsing, `SteamLobbyService` in Electron main/preload, renderer `steamLobbyClient.js`, wired into `multiplayerLobby.js` + invite button). Room codes remain available for browser dev/LAN/QA. **Not yet live-verified** — needs two real Steam accounts, which this sandbox doesn't have |
