...
Last reconciled: 2026-09-23 · source baseline `mothership` `d12b9c4`; co-op follow-up `8da53de` and Deck/PvP follow-up `2d0032a` on `dev/sprint-46`; one physical packaged capture `v2.4.11-beta` `aafe429fae34` on Steam Deck. The package predates `2d0032a` and is one client’s evidence, not release acceptance; see the [evening session analysis](docs/reports/session-log-analysis-2026-09-23-deck-pvp-session.md).
...
| PvP | One physical Deck capture shows Steam lobby/ready/deploy, remote-avatar readiness, inbound `pvp-rival` damage, one death, and respawn. Current source `2d0032a` fixes PvP at four hearts independent of fatigue/loadout and adds client skip/server validation rejection for PvP leaderboard submission; focused unit tests pass. | Not launch-ready: the capture predates that source fix. Require paired bidirectional hit/result/reconnect and deployed-backend evidence; decide rewards, spawn protection, doors, and PvE-ruleset behavior. |
...
| Performance diagnostics | GPU queries reset cleanly across transitions; frame pacing percentiles, world-model load latency, and Deck-safe diagnostic context are exported. The evening Deck capture sampled 0–64 transient effects (final 13). Current source `2d0032a` unit-tests world-render suspension behind game-over and a stable shadow-map key. | Physical Deck pacing fails the captured build (p50 84.7 ms; max 4.614 s). The current source mitigations need a matched packaged re-benchmark; causal trace, quality-tier decision, and physical sign-off remain open. |
