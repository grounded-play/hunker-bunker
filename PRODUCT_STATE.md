...
| Steam Lobby (Friends/Invite/Join Game) | Code-complete for same-region create/browse/join/invite, with callsign/loadout/ready-up fixes, client host-state synchronization, authoritative ready snapshots, host-only deploy enforcement, and the pre-existing-lobby invite sequencing race fixed. Cross-region discovery remains blocked by the installed `steamworks.js` binding's missing distance filter; real two-account Steam acceptance is still open. Meetup path: launch both packaged builds from Steam, host creates CO-OP lobby, host invites guest, both ready, then host selects START SQUAD. See `docs/steam-lobby-integration-plan-2026-08-20.md` and `docs/multiplayer-host-assignment-review-2026-08-20.md` |
