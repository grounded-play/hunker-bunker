...
Last reconciled: 2026-09-23 · source baseline `mothership` `d12b9c4`; co-op follow-up implementation `8da53de` on `dev/sprint-46`; one physical packaged capture `v2.4.11-beta` `aafe429fae34` on Steam Deck. The package capture is one client’s evidence, not release acceptance; see the [evening session analysis](docs/reports/session-log-analysis-2026-09-23-deck-pvp-session.md).
...
| PvP | One physical Deck capture shows Steam lobby/ready/deploy, remote-avatar readiness, inbound `pvp-rival` damage, one death, and respawn. Generic run-leaderboard submission lacks a multiplayer-mode guard in source. | Not launch-ready: require paired bidirectional hit/result/reconnect evidence; decide fatigue/HP, rewards, spawn protection, doors, and PvE-ruleset behavior; segregate PvP leaderboards. |
...
| Performance diagnostics | GPU queries reset cleanly across transitions; frame pacing percentiles, world-model load latency, and Deck-safe diagnostic context are exported. The evening Deck capture sampled 0–64 transient effects (final 13). | Physical Deck pacing fails the current capture (p50 84.7 ms; max 4.614 s). A matched packaged route, causal trace, quality-tier decision, and physical sign-off remain open. |
