...
| Steam Lobby (Friends/Invite/Join Game) | Not implemented — relay/room-code is the only join path today; in progress, see `docs/sprint26-master-plan-2026-08-19.md` |
...
| Steam Stats/Cloud | Fixed 2026-08-20: all 8 dashboard stats now synced (`src/steamStats.js`, was 2/8); Steam Cloud save bridge now wired (`src/steamCloudSaveBridge.js` — was fully built in Electron but never called, so Cloud synced nothing). Not yet verified against a real installed build/Steam Cloud round-trip. Still needs your dashboard check: Cloud byte/file quota values, "developers only" checkbox, Steam Input default controller config selection |
