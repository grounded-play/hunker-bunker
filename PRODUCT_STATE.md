...
| Steam Lobby (Friends/Invite/Join Game) | Code-complete, all 4 implementation steps of `docs/steam-lobby-integration-plan-2026-08-20.md` shipped. First real playtest (2026-08-20, later) found and fixed two real gaps: lobbies were FriendsOnly (never appear in `getLobbies()`, made public browsing structurally impossible — fixed, now public by default with a real lobby browser UI) and Invite Friends silently no-opped when the Steam overlay wasn't attached (fixed with an honest `launchedViaSteam`-based warning, since the overlay categorically can't work if the binary wasn't launched through Steam). Room codes remain available for browser dev/LAN/QA. **Still not live-verified** — needs two real Steam accounts, which this sandbox doesn't have |
...
| Performance | `docs/perf-chunk-mount-plan-2026-08-20.md`: real chunk-mount batch-stacking during deploy fixed (time-budget scheduling) and live-verified. Separately found and fixed a real diagnostic bug: the long-task attribution tag went stale during idle menu time and silently misattributed unrelated freezes to chunk-mount, including the previously-observed ~6s freeze. That freeze is real and still unexplained — likely the same one flagged in Sprint 26's backlog — but is no longer mis-blamed on chunk mounting; next investigation has trustworthy data to work from |
