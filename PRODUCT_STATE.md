...
| Performance | Prior chunk-mount batching, stale attribution, and collapsed-container render waste are fixed. The log13 worktree holds shader programs constant, prewarms normal/adaptive variants, and removes measured update-loop bottlenecks; controlled frame time improved 51.40ms→29.80ms. Hardware diagnostics now request the high-performance GPU, asynchronously measure real GPU frame time when supported, identify software rendering, and estimate unique texture/geometry/render-target bytes on a cached cadence. The validation scene estimated 784.4 MiB potential allocation, 757.1 MiB from textures, making KTX2/Basis the next measured memory target. Deck adaptive mode still preserves chunks/actors while shedding postprocessing, shadows, and pixel ratio. A packaged physical-PC/Deck capture remains the acceptance gate. See `docs/gpu-memory-utilization-plan-2026-08-21.md`, `docs/log13-gameplay-fps-plan-2026-08-20.md`, and Sprint 28 Lane F. |
