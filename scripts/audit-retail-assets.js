...
// video and 3D-model weight. Lowered 1150->950 MiB: the .glb model fix
// (docs/steam-lobby-integration-plan-2026-08-20.md sibling bugfix -- 3D
// models were shipping fully inside app.asar, which broke loading them in
// packaged builds) moved ~259MB of runtime .glb data out of app.asar and
// into app.asar.unpacked, so measured app.asar dropped from ~1019-1029 MiB
// to ~760 MiB (a real `--package` Linux build measured 797720366 bytes).
// This budget has been bumped up before at the same real-growth cadence as
// PUBLIC_BUDGET (600 -> 750 -> 950 -> 1150); this drop is the first time
// it's moved the other way, for the same reason PUBLIC_BUDGET doesn't
// track this weight at all -- it was never real asar payload to begin
