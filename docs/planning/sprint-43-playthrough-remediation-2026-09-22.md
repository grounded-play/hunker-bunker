...
These are observations from the 2026-09-22 player playthrough and the provided Armory screenshot, cross-referenced with current source and the existing Sprint 41 documentation. The newly mentioned session log was **not present in the conversation and was not retrieved from the authenticated log server**. Do not represent it as reviewed. Sprint 43 issue [#86](https://github.com/grounded-play/hunker-bunker/issues/86) owns retrieval and timestamped triage. The 2026-09-11 hosted-session report is historical evidence from older builds, not proof of today's failures. Automated tests also do not constitute human/physical Steam Deck or two-Steam-account acceptance.
...
## Fresh log intake (pending)

The current repo runbook is [`docs/session-log-review-runbook.md`](../session-log-review-runbook.md). Request the playtester upload **the actual just-created log JSON or the hosted server filename** (and both client logs for co-op), plus build SHA, device/resolution, gameplay mode and rough reproduction timestamps. Authorized local retrieval: `npm run logs:fetch -- --latest 5` or `npm run logs:fetch -- --name <server-filename>`; analyze with `npm run logs:analyze -- logs/<file>.json`. Add a dated redacted summary under `docs/reports/`, link evidence to [#86](https://github.com/grounded-play/hunker-bunker/issues/86) and each proven symptom, and update this plan if the capture changes scope. **No log evidence is claimed at plan creation.**
