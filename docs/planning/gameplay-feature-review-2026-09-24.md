...
| Slice probes, 3 seeds × 3 classes, before/after | Before 9/9 (`030d773`), after 9/9 (`e1d47fd`), **integrated 9/9 (`efcb5ae`)** — real fights and drops through both contracts; see the slice report | `tests/e2e/probes/slice.spec.js`; raw output and screenshots in `docs/reports/assets/sprint-47/`. No fight or drop happened in-game yet (no Lane 2/3 contract at runtime), so the per-class build comparison has no in-game data |
...
8. **Lane 2's formation damage rule is not in the game.** `encounterDamageMultiplier` (anchor/support shielding) is used only by `simulateEncounterPriority`; the runtime never calls it, so `audit:combat-encounters`' priority table models a rule players never meet. Open for Lane 2: apply it in the runtime or label the table as design intent.
9. **A Scout can open the bait without the ammunition to finish it.** In the integrated slice probe (seed 99991) the Scout, starting sidearm and empty reserve, ran dry against Locked Crossfire (SIDEARM 0/6, CACHE 0/54) and did not clear it in 240 s; the Tank cleared the same fight in 40 shots. The probe does not move or reload, so this needs a human run before it is called a balance bug.
