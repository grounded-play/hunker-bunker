...
## September 10 user-directed patch, navigation and atmosphere revision

Rebase work on c7a663d and preserve concurrent server work. Replace four flat patch placeholders and the duplicated Gold Seal with five original transparent designs; retain item IDs/unlocks. Correct the chest-depth cap and use a 7.4mm clearance at 1.85m operator scale, retaining physical backing, bone parenting and depth occlusion. Unify the navigation display with the full blueprint renderer, player-centered at 5Hz; move the existing telemeter DOM into the map panel/sidebar rather than duplicate its state. Reduce HUD panel bulk and fix short-screen spacing. Lift perspective camera toward a tactical overhead angle, strengthen normalized tilt blur beyond a protected player/aim band, and restore close night fog to hide distant world cutoffs. Validate art alpha, three class mounts, picker fit, map discovery consistency, actual gameplay screenshots, tests/lint/build/generated gates. Do not include unrelated server edits in this branch update.
