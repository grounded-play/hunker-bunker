...
1. **Steamworks Isolation**:
   - `steamworks.js` is only loaded in the Electron main process ([`electron/main.cjs`](../../electron/main.cjs)).
...
2. **Socket.IO Relay Protocol**:
   - Relay server: [`server/relay.js`](../../server/relay.js).
   - Client lobby: [`src/multiplayerLobby.js`](../../src/multiplayerLobby.js).
...
3. **Steam Inventory Economy**:
   - Schema source of truth: [`steam/inventory_schema_hunker_bunker.json`](../../steam/inventory_schema_hunker_bunker.json).
   - Catalog runtime: [`src/data/steamItemCatalog.js`](../../src/data/steamItemCatalog.js).
