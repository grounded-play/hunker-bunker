...
| :--- | :---: | :--- | :--- | :--- |
| GAP-RN-10 | P0 | Deck frame pacing remains far below release acceptance; transition/program correlations need a causal trace. | 3,600 retained gameplay intervals: p50 84.7 ms, p95 223.6 ms, p99 519.5 ms, max 4.614 s; 428 PERF diagnostic windows. `shadowMap.enabled` is a shader key, but that is a hypothesis rather than the captured root cause. | Sticky `shadowMap.enabled = true` key + `autoUpdate` toggle implemented; world rendering suspended during game-over. **#52 partial (hardware verification open).** |
| GAP-RN-11 | P1 | Game-over/gameplay-profile transition has an unmeasured rendering/workload risk. | During game-over, `gameplay` profile and 28 chunks remain present for 11 816–1,679 ms diagnostic windows over ~34 s. | World render suspension implemented during `#game-over-modal` to avoid rendering hidden geometry. **#52 partial (hardware verification open).** |
...
| GAP-PV-05 | P2 | PvE missions and run cards appear in PvP. | Mapping objective and `camp_paranoia` cards observed. | Bypassed PvE missions and run modifier draws in PvP (`resetRunToStartingState` and `setupMultiplayerNetwork`); HUD cards hidden. Unit-tested. **#51 partial.** |
...
| GAP-GP-13 | P1 | Deck input provenance cannot certify a controller-only route. | 607 total `fire-input` events use `pointer`; final `lastInputMode` is `keyboard`. A Deck controller is present, but Steam Input can synthesize these events. | Fixed controller fire trigger to pass `source: 'controller'`; added `source` to `shot-accepted`; added active action-set logging on deploy and input mode change; included in session capture diagnostics. Unit/E2E-tested. **#53 partial (hardware verification open).** |
