...
| GAP-PV-01 | **Partial / P0 authority mismatch.** Local PvP vitals now target four hearts, while the relay and fresh remote replicas still initialize at three. | `2d0032a`, `4da77cc`; `server/relay.js`, `src/threeGame.js` |
...
| GAP-PV-01 | P0 | Local campaign fatigue reaches PvP health. | The captured package enters PvP at 2/2 after solo deaths. Current source sets local PvP vitals to four, but `server/relay.js` `PVP_DEFAULT_MAX_HP` and fresh remote replicas still initialize at three. | **Partial implementation / regression risk.** Define one shared authority value, update all participants, and add an integration test that proves the fourth hit is the only lethal one. **#51 open.** |
