...
| GAP-PV-01 | P0 | Local campaign fatigue reaches PvP health. | The local Deck enters PvP at 2/2 after solo deaths; source applies fatigue modifiers without a PvP exclusion. | Symmetric 4-heart loadout implemented; campaign fatigue excluded; fresh 4 HP restored on PvP entry. Unit-tested in `threeGame.pvpVitals.test.js`. **#51 partial (hardware verification open).** |
| GAP-PV-02 | P1 | Spawn/respawn fairness needs a paired test. | One incoming PvP death, respawn start at (9, 3), and relocation/depenetration occur locally. | 3.0s spawn invulnerability window implemented on PvP respawn; damage blocked while active. Unit-tested. **#51 partial (hardware verification open).** |
| GAP-PV-03 | P1 | PvP death can trigger Black Box recovery and objective XP. | Local Black Box recovery occurs about 8 seconds after respawn, then +50 objective XP; `handleDeath` has no PvP guard. | PvP death guarded: no Black Box recorded, no marker dropped, no XP farm. Unit-tested. **#51 partial.** |
...
| GAP-PV-06 | P2 | Door-event churn needs reproduction. | Ten door-toggle events in 19 seconds, including repeated remote traffic. | Monotonic sequence (`_bunkerBlastDoorSequence`) and 400ms debounce implemented; stale remote sequences dropped. Unit-tested. **#51 partial.** |
| GAP-PV-07 | P1 | Outgoing PvP hit result is not observable from the current log. | 21 accepted PvP shots/projectiles; no recipient health, relay verdict, or kill event; `rivalKills` 0. | `pvp-hit-dealt` and `pvp-hit-confirmed` telemetry added to weapon hit and damage handlers. **#51 partial.** |
