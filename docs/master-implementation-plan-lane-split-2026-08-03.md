...
- 2026-08-03: Antigravity/Gemini Phase 8.1 & 8.2 **implemented, connected, and automated**.
  Author authored `docs/camp3-boss-climax-design.md` detailing the Camp-3 Vesper perimeter siege, Corrupted Operator phase transitions, faction verb synergy (Route Intel, Triage, Field Resupply), and launch boarding vector consequences.
  Wired Faction Active Verb UI biome prompts (`showBiomePrompt`) and feedback handlers (`camp-verb-activated`, `camp-verb-denied`) into `main.js`.
  Added `src/campActiveVerbUi.test.js` covering active verb catalog integrity, resource gates, cooldowns, and degradation states.
  Full Vitest test suite 237/237 files (1,959 tests) passing green.

- 2026-08-03: Codex Phase 2.1 **release gates completed**. The existing
  release path already embedded and verified version, commit, branch, dirty
  state, and the operator-supplied Steam build identifier, but clean-tree
  enforcement applied only to uploads and could be bypassed on upload with
  `--allow-dirty`. `steam:prepare` now rejects dirty release inputs by default,
  while `--allow-dirty` is explicitly limited to non-upload local QA. Depot
  auditing now rejects `backend.env`-style environment files and SQLite/DB
  main, WAL, and shared-memory artifacts in addition to the existing
  `steam_appid.txt`, secret-key, and JSON-backend exclusions. Focused depot
  tests cover the new release-input exclusions. A real clean Windows/Linux
  package and Steam upload/install pass remain operator acceptance work.
