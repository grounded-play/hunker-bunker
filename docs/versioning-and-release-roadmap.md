...
| **v2.4.12-beta (Staged)** | Sprint 47 (`dev/sprint-47`) | 2026-09-24 | Release PR to `mothership` | **Invisible Essentials Phase 4 & Milestone Closures**: pneumatic transit return network, milestone boss defeat extraction terminal unlocking, sanctuary teleport, tactical map breadcrumbs; verification suites for Persistence (#78), Fabrication Bay (#80), Hero Selection (#81), and Armory (#82). 4,201 tests across 469 files (100% green). [Release notes](releases/v2.4.12-beta.md). |
| **v2.4.11-beta (Staged)** | Sprint 45.1 (`dev/sprint-45`) | 2026-09-23 | Release PR to `mothership` | **Steam Deck playthrough hotfixes**: tactical-map action set and toggle debounce, extraction objective after the last objective or third boss, shared shockwave/pip/splash resources, ship-module collision, co-op boss hits reach the host, route reachability validation, E2E gameplay specs green, every menu control reachable by controller, dev-only Electron debugging. 3,995 tests across 437 files. Not verified on Deck hardware or two-account co-op. [Release notes](releases/v2.4.11-beta.md). |
| **v2.4.10-beta (Staged)** | Sprint 45 (`dev/sprint-45`) | 2026-09-23 | Branch commits on `dev/sprint-45` | **Seeded expeditions in a persistent campaign**: 5 conditions with gameplay effects, kill signatures and atmosphere; per-deployment corridor rubble; six-room camp/hive compounds with room title cards and hive encounter profiles; versioned route generation (old saves keep their geography); per-campaign gate challenges; world transformations (canyon bridge deck, fortified perimeters, hive bond/harvest outcomes); scan fog-of-war and grouped hazard zones; HUD overlap fixes; cursor hidden in cinematics; Deck title-screen frost; crossings proven walkable (five gate defects fixed); overnight creep and camp condition with gameplay effects; campaign-rolled objective packages for all four ship goals. 3,958 tests across 429 files. Not verified on Deck hardware, co-op or Cloud. [Release notes](releases/v2.4.10-beta.md). |
