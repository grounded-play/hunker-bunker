...
| **v2.4.11-beta (Staged)** | Sprint 45.1 (`dev/sprint-45`) | 2026-09-23 | Release PR to `mothership` | **Steam Deck playthrough hotfixes**: tactical-map action set and toggle debounce, extraction objective after the last objective or third boss, shared shockwave/pip/splash resources, ship-module collision, co-op boss hits reach the host, route reachability validation, E2E gameplay specs green. 3,990 tests across 436 files. Not verified on Deck hardware or two-account co-op. [Release notes](releases/v2.4.11-beta.md). |
