...
4. Check runtime logs for structured request entries:
...
The purchase ledger lives in `db_storage.json` under `purchases`. It is now a
canonical state table, not an append-only list. Each record is keyed by
...
1. Find the transaction by `transId`, `orderId`, or `requestId` in
   `db_storage.json`.
...
JSON-on-volume is acceptable for local development and a single-machine beta,
but it is not the final production economy store. The migration path should be:

1. Add a small SQLite dependency such as `better-sqlite3` or `sqlite`.
2. Create `server/db-sqlite.js` with the same public functions as `server/db.js`.
3. Use tables:
...
4. Add an offline migration script that reads JSON once and writes SQLite in a
   transaction.
