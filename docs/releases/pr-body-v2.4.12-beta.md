...
| **QA-DECK-INPUT** | **Steam Deck double presses and carry-over** | **Fixed — confirm on Deck** | One press reached the game twice (Chromium Gamepad API first, native Steam Input a moment later; the Deck log shows Escapes in pairs ~0.1 s apart): B closed the map and then opened the pause menu, the ☰ settings button flashed open and shut, and a button held while closing a menu carried into gameplay as a sprint. `src/controllerPressGate.js` lets one press act once, in the action set it started in; unit-tested, with a browser reproduction probe (`tests/e2e/probes/deck-double-press.spec.js`). Left/right inversion on the Deck is not fixed yet — which control is inverted is being confirmed with the owner. |
