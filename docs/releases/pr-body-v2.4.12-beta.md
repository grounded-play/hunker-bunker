...
2. Confirm the look: depth-of-field/tilt-shift, moving shadows, 3D enemies and 3D props stay on throughout — including after the game logs `adaptive-gameplay-quality-engaged` (it should now only lower resolution; the log line should read `"postprocessing": true`).
