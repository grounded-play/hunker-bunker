...
This release delivers **Phase 4 of the Invisible Essentials plan**, resolves critical multiplayer networking and map lifecycle rules identified in the **2026-09-24 Steam Deck + PC co-op QA session**, overhauls the **Minimap Radar Scan, Fog of War, and Wavefront Dissipation**, fixes the **Steam Deck controls** (double presses, B opening settings behind the map, inverted right-stick camera), **restores full visual quality** (post-processing, shadows, 3D enemies and props on every machine) while recovering frame time on the CPU, and automates code-side verification suites across all core milestone tickets.
...
| **PLAN-HUD-MAP** | **Minimap Progressive Radar Reveal, Dissipation & Fog of War** | **Closed (Full Implementation)** | Closes instant-reveal and line truncation defects. Implements progressive wavefront reveal, 400ms dissipation tail with quadratic fade-off, minimap bezel mask, and high-contrast tactical CRT Fog of War pattern (`05c4300`). The same commit also carried the title freeze (fixed in `6ea3506`) and the low-FPS quality cuts (reverted in `cfbfaff`, see QA-PERF-QUALITY). |
...
| **QA-DECK-INPUT** | **Steam Deck double presses and carry-over** | **Fixed — confirm on Deck** | One press reached the game twice (Chromium Gamepad API first, native Steam Input a moment later; the Deck log shows Escapes in pairs ~0.1 s apart): B closed the map and then opened the pause menu, the ☰ settings button flashed open and shut, and a button held while closing a menu carried into gameplay as a sprint. `src/controllerPressGate.js` lets one press act once, in the action set it started in. The browser reproduction probe (`tests/e2e/probes/deck-double-press.spec.js`, fake pad + stubbed native bridge) showed the first version only half-fixed it: the tactical map polls the pad itself, so the gate never saw that press and native's copy opened settings (5/5 runs, before and after). Now a press the map consumes is held in the gate until the pad lets go, however late native's copy arrives (`c0b5ab0`, `f1c6faa`); probe: map closes, no dash, no settings in 2/2 runs with native arriving 1.7–3.3 s later ([results](../reports/assets/qa-2026-09-24/deck-double-press-probe.jsonl)). |
| **QA-DECK-CAMERA** | **Right-stick camera turning inverted on Steam Deck** | **Fixed — confirm on Deck** | Pushing the right stick right turned the view left, in both isometric and third-person. The camera's orbit angle grows opposite to screen direction; the mouse edge turn already accounted for that, the stick did not. The stick now uses the same sign, for every controller (native Steam Input and a browser gamepad report the axis the same way; the Deck config has no invert). The mouse is unchanged. Test: the view's forward swings toward its old right in both modes (`src/threeGame.facingYaw.test.js`, `0bbb438`). |
...
- Reconciles repository state; the suite now stands at **4,279 tests across 483 files** (100% green).

---

## 📜 Commit Ledger (Sprint 46 → Sprint 47 & Release Hardening)

A selection: the code changes and key documents. `git log origin/mothership..` has all 78 commits.

| Commit | Category | Description |
| :--- | :--- | :--- |
| `0bbb438` | fix(input) | Right stick turns the camera right |
| `f1c6faa` | fix(input) | B that closes the map no longer opens settings behind it |
| `cfbfaff` | perf | Keep full visual quality; win the frame time back on the CPU |
| `c0b5ab0` | fix(input) | One Steam Deck press acts once, in the context it started in |
| `31872fe` | fix(debug) | 'ARMORY ALL' unlocks all Armory equipment, polishes and sheens |
| `d6ed49f` | fix(ci) | Detect Lighthouse by query parameter as well as user agent |
| `6ea3506` | fix(ui) | Stop the title-screen freeze from the presentation-cursor observer |
| `fcce193` | fix(campaign) | Respawn builds the run's map, not the campaign's |
...
| `2d2dfd4` | fix(report) | Fill names before translating item labels (no raw {drop}) |
| `c6d984d` | fix(security) | Resolve CodeQL remote property injection in sessionLogSampler |
...
- [x] **Tested** — **4,279 tests across 483 files pass (100% green)**, plus browser probes for the map rule, the Deck double press and the render/CPU profile.
...
- `npm test`: 4,279 passed (483 files).
- `npm run i18n:audit`: unchanged — 0 unlocalized DOM strings across 7 locales.
- `npm run build`: `vite build` completed in 2.40s; required media audited.
...
### 5. Steam Deck Controls (QA-DECK-INPUT, QA-DECK-CAMERA)
1. Open the tactical map, press **B**: the map closes and nothing else happens (no pause/settings menu, no dash).
2. Press the **☰** settings button: settings open and stay open.
3. Hold a button (e.g. sprint) while quitting a menu: the character does not start sprinting until it is pressed again.
4. Push the **right stick right**: the camera turns right (both camera modes). The mouse on PC should feel unchanged.

### 6. Visual Quality & Frame Time (QA-PERF-QUALITY)
1. Play 5+ minutes on the Deck and on the PC.
2. Confirm the look: sharp full-resolution image, shadows in the suit light, depth-of-field/tilt-shift, moving shadows, 3D enemies and 3D props stay on throughout — including after the game logs `adaptive-gameplay-quality-engaged` (it should now only lower resolution; the log line should read `"postprocessing": true`).
3. Export the session log: compare gameplay frame intervals with `logs/…2026-09-25T06-27-40-524Z-mugkws10-6ryp.json` (PC, ~48 ms average before this change).
