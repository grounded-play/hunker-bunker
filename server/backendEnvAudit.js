...
import { STEAM_LEADERBOARD_DEFS } from './leaderboardScoring.js';

export const EXPECTED_STEAM_APPID = 4957040;
export const REQUIRED_BACKEND_ENV_VARS = Object.freeze([
...
export function isBackendEnvEnabled(value) {
...
export function auditSteamBackendEnv(env = process.env, { strict = isBackendEnvEnabled(env.HB_BACKEND_AUDIT_STRICT) || env.NODE_ENV === 'production' } = {}) {
    const failures = [];
    const warnings = [];
    ...
    const autoCreateLeaderboards = isBackendEnvEnabled(env.HB_STEAM_LEADERBOARD_AUTO_CREATE);
    const microtxnEnabled = isBackendEnvEnabled(env.HB_STEAM_MICROTXN_ENABLED);
    const storeEnabled = isBackendEnvEnabled(env.HB_STEAM_STORE_ENABLED);
    const mockStorePurchases = isBackendEnvEnabled(env.HB_STEAM_STORE_MOCK_PURCHASES);
    ...
    if (!publisherKey) addIssue(strict ? failures : warnings, 'missing_publisher_key', 'HB_STEAM_PUBLISHER_KEY is required for live auth, inventory, and leaderboard writes.');
    if (!sessionSecret) addIssue(strict ? failures : warnings, 'missing_session_secret', 'HB_SESSION_SECRET should be explicit so session tokens are not signed with the publisher key or dev fallback.');
    ...
    if (!dbPath) {
        ...
        if (!path.isAbsolute(dbPath)) addIssue(failures, 'relative_db_storage_path', 'HB_DB_STORAGE_PATH must be an absolute path.', { value: dbPath });
        if (strict && /(^|\/)server\/db_storage\.json$/.test(dbPath)) addIssue(failures, 'default_db_storage_path', 'Strict deploys must not use the repo-local server/db_storage.json path.', { value: dbPath });
    }
    ...
    if (strict && autoCreateLeaderboards) addIssue(warnings, 'leaderboard_auto_create_enabled', 'Leaderboard auto-create is enabled in strict mode; verify this is intentional for the target Steam app.');
    if (storeEnabled && !microtxnEnabled) addIssue(failures, 'store_without_microtxn', 'HB_STEAM_STORE_ENABLED=1 requires HB_STEAM_MICROTXN_ENABLED=1.');
    if (microtxnEnabled && !publisherKey) addIssue(failures, 'microtxn_without_publisher_key', 'HB_STEAM_MICROTXN_ENABLED=1 requires HB_STEAM_PUBLISHER_KEY.');
    if (strict && mockStorePurchases) addIssue(failures, 'mock_store_in_strict', 'HB_STEAM_STORE_MOCK_PURCHASES must not be enabled in strict production deploys.');
    ...
    return {
        ...
        requiredEnv: REQUIRED_BACKEND_ENV_VARS,
...
export function formatBackendEnvIssue(issue) {
